...
//adugare in lista dubla circulara
void adaugareNodInLista(Nod** cap, Evaluation* e) {
	if (*cap == NULL) {
...
//afisare lista dubla circulara
void parcurgereLista(Nod* cap) {

	Nod* temp = cap;
	while (temp->next != cap) {
		afisareEvaluation(temp->info);
		temp = temp->next;
	}
	afisareEvaluation(temp->info);
}
...
int nrExam(Nod* cap) {
	...
	Nod* temp = cap;
	while (temp->next != cap) {
		if (cautareString(charsArr, j, temp->info->examName) == 0) {
			charsArr[j] = temp->info->examName;
			j++;
			nr++;
		}
		temp = temp->next;
...
int* numarTotalStudenti(Nod* cap) {
	...
	for (int i = 0; i < j; i++) {
		...
		l = 0;
		while (tmp->next != cap) {
			if (strcmp(tmp->info->examName, charsArr[i]) == 0) {
				l++;
			}
			tmp = tmp->next;
		}
